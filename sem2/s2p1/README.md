...
3. Реализовать внешнюю функцию make_тип(), где тип – тип реализуемой структуры. Функция должна получать значения для
   полей структуры какпараметры функции и возвращать структуру как результат. При передаче ошибочных параметров следует
...
> distance(double x1, doudle y1) – вычисление расстояния от точки с координатами (first, second) до точки с координатами (
> x1,y1).
...
Для доступа к приватным полям `name` и `group` класса `Student` можно спользовать открытые (публичные) методы, такие как
